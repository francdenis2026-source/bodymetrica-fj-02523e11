    ...
    <div className="min-h-[100dvh] bg-background text-foreground">
      <header className="relative z-50 border-b border-border/70 bg-background/95">
        <div className="container mx-auto flex h-16 max-w-7xl items-center justify-between px-4 md:px-6">
          ...
          {isLoggedIn ? (
            <Button asChild size="sm" variant="outline" className="min-h-11 rounded-xl bg-background px-4 active:scale-[0.98]"><Link to="/dashboard">Abrir painel</Link></Button>
          ) : (
            <Button asChild size="sm" variant="ghost" className="min-h-11 rounded-xl px-4 active:scale-[0.98]"><Link to="/auth" search={{ registerMode: false, reset: false, name: "", birthDate: "", goal: "", weight: "", height: "", activityLevel: "" } as any}>Entrar</Link></Button>
          )}
        </div>
      </header>

      <main className="relative isolate min-h-[calc(100dvh-4rem)] overflow-hidden">
        <img src="https://images.unsplash.com/photo-1534438327276-14e5300c3a48?auto=format&fit=crop&q=80&w=1800" alt="" aria-hidden="true" className="absolute inset-0 -z-30 h-full w-full object-cover object-center" />
        ...
        <section className="container mx-auto flex min-h-[calc(100dvh-4rem)] max-w-7xl flex-col px-4 md:px-6">
          <div className="grid flex-1 items-center gap-8 py-7 md:py-9 lg:grid-cols-[1.08fr_0.92fr] lg:gap-12 lg:py-8">
            <div className="max-w-2xl">
              {isLoading ? (
                ...
                <div>
                  <div className="mb-5 inline-flex items-center gap-2 rounded-xl border border-primary/20 bg-background/92 px-3.5 py-1.5 text-sm font-semibold text-foreground shadow-sm"><span className="size-2 rounded-sm bg-primary" aria-hidden="true" />Sua evolução em uma única visão</div>
                  <h1 className="max-w-3xl font-display text-[clamp(2.55rem,6.4vw,4.45rem)] font-semibold leading-[1.01] tracking-[-0.045em] text-balance">Entenda seu corpo. <span className="text-primary">Acompanhe o que realmente muda.</span></h1>
                  <p className="mt-5 max-w-[37rem] text-base font-medium leading-7 text-foreground/78 md:text-lg md:leading-8">Composição corporal, alimentação, hidratação e treino reunidos em uma experiência direta, visual e fácil de acompanhar todos os dias.</p>
                </div>
              )}
              ...
              <div className="mt-7 flex flex-col gap-3 sm:flex-row sm:items-center">
                {isLoading ? <><Skeleton className="h-12 w-full rounded-xl sm:w-44" /><Skeleton className="h-12 w-full rounded-xl sm:w-40" /></> : (
                  <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row"><QuickOnboarding isLoggedIn={isLoggedIn} /><Button asChild variant="outline" size="lg" className="h-12 rounded-xl border-border/85 bg-background/92 px-6 font-medium shadow-sm hover:border-primary/30 hover:bg-background active:scale-[0.98]"><Link to="/about" className="group gap-2">Conhecer o projeto<ArrowRight size={16} /></Link></Button></div>
                )}
              </div>
              ...
              <div className="mt-7 grid max-w-2xl overflow-hidden rounded-2xl border border-border/80 bg-background/94 shadow-sm sm:grid-cols-3">
                {HOME_FEATURES.map(({ icon: Icon, eyebrow, title, description }, index) => (
                  <div key={title} className={`p-4 hover:bg-primary/[0.035] ${index > 0 ? "border-t border-border/70 sm:border-l sm:border-t-0" : ""}`}><div className="flex items-center gap-2.5"><div className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary" aria-hidden="true"><Icon size={18} strokeWidth={2} /></div><span className="text-[10px] font-bold tracking-[0.14em] text-primary/80">{eyebrow}</span></div><h2 className="mt-3 font-display text-base font-semibold tracking-tight">{title}</h2><p className="mt-1 text-xs leading-5 text-foreground/62">{description}</p></div>
                ))}
              </div>
            </div>
            ...
            <div className="relative mx-auto w-full max-w-[430px] lg:max-w-[480px] xl:max-w-[510px]">
              <div className="overflow-hidden rounded-[1.75rem] border border-white/20 bg-card shadow-lg shadow-black/15 ring-1 ring-black/5">
                <div className="on-media relative aspect-[4/4.7] overflow-hidden bg-muted sm:aspect-[4/4.6] lg:aspect-[4/4.45]">
                  ...
                  <div className="absolute right-4 top-4 rounded-2xl border border-white/20 bg-black/55 p-3 text-white shadow-sm md:right-5 md:top-5"><p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-white/65">Acompanhe</p><div className="mt-2 flex items-center gap-3 text-xs font-medium"><span className="flex items-center gap-1.5"><Target size={14} /> Metas</span><span className="h-4 w-px bg-white/20" /><span className="flex items-center gap-1.5"><BarChart3 size={14} /> Evolução</span></div></div>
                  <div className="absolute inset-x-0 bottom-0 p-5 text-white md:p-6"><div className="mb-3 h-px w-12 bg-primary" aria-hidden="true" /><p className="text-xs font-medium text-white/75 md:text-sm">Acompanhamento que cabe na rotina</p><p className="mt-1.5 max-w-sm font-display text-xl font-semibold leading-tight text-balance md:text-2xl">Dados organizados para transformar registros em progresso visível.</p><div className="mt-4 flex flex-wrap gap-2 text-[11px] font-medium text-white/85"><span className="rounded-lg border border-white/15 bg-black/25 px-2.5 py-1">Corpo</span><span className="rounded-lg border border-white/15 bg-black/25 px-2.5 py-1">Nutrição</span><span className="rounded-lg border border-white/15 bg-black/25 px-2.5 py-1">Hidratação</span><span className="rounded-lg border border-white/15 bg-black/25 px-2.5 py-1">Treino</span></div></div>
                </div>
              </div>
            </div>
          </div>
          ...
          <footer className="flex flex-col gap-2 border-t border-border/65 py-3.5 text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
            ...
            <nav className="flex flex-wrap items-center gap-x-1" aria-label="Rodapé">
              ...
              <Link to="/admin/login" className="inline-flex min-h-10 items-center rounded-lg px-2 text-[11px] text-muted-foreground/55 transition-colors hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" aria-label="Acesso institucional">Acesso institucional</Link>
            </nav>
          </footer>
        </section>
      </main>
