...
const registerSchema = z.object({
  ...
  email: z.string().trim().max(254, "E-mail muito longo").superRefine((value, ctx) => {
    const message = getEmailValidationError(value);
    if (message) ctx.addIssue({ code: z.ZodIssueCode.custom, message });
  }),
  cpf: z.string().refine(isValidCpf, "CPF inválido. Confira os 11 dígitos."),
...
function RegisterPage() {
  ...
  async function goNext() {
    ...
    const normalizedCpf = normalizeCpf(form.getValues("cpf"));
    const normalizedEmail = normalizeEmail(form.getValues("email"));

    const [{ data: cpfOwner }, { data: emailOwner }] = await Promise.all([
      (supabase as any).from("profiles").select("id").eq("cpf", normalizedCpf).maybeSingle(),
      (supabase as any).from("profiles").select("id").eq("email", normalizedEmail).maybeSingle(),
    ]);

    if (cpfOwner) {
      form.setError("cpf", { message: "Este CPF já está vinculado a uma conta." });
      return;
    }
    if (emailOwner) {
      form.setError("email", { message: "Este e-mail já está vinculado a uma conta." });
      return;
    }
  ...
  async function onSubmit(values: RegisterValues) {
    ...
    try {
      ...
      if (!isValidCpf(normalizedCpf) || emailValidationError) {
        setStep(1);
        if (!isValidCpf(normalizedCpf)) form.setError("cpf", { message: "CPF inválido. Confira os 11 dígitos." });
        if (emailValidationError) form.setError("email", { message: emailValidationError });
        return;
      }
      ...
      if (cpfOwner || emailOwner) {
        setStep(1);
        if (cpfOwner) form.setError("cpf", { message: "Este CPF já está vinculado a uma conta." });
        if (emailOwner) form.setError("email", { message: "Este e-mail já está vinculado a uma conta." });
        return;
      }
      ...
      if (error) {
        ...
        if (message.includes("already") || message.includes("registered") || message.includes("exists")) {
          ...
          navigate({ to: "/auth" as any, search: { email: normalizedEmail } as any });
          return;
        }
        ...
        <div className="grid w-full overflow-hidden rounded-[1.75rem] border border-border/90 bg-card/96 shadow-[0_28px_90px_rgba(0,0,0,.22)] ring-1 ring-foreground/5 lg:h-[min(720px,calc(100vh-32px))] lg:grid-cols-[0.74fr_1.26fr]">
          <section className="on-media relative hidden overflow-hidden lg:block">
            ...
            <div className="relative flex h-full flex-col justify-between p-6 text-white xl:p-7">
              <Link to="/" className="inline-flex w-fit items-center gap-2.5 rounded-xl border border-white/15 bg-black/35 px-3 py-2 backdrop-blur-md"><span className="flex size-8 items-center justify-center rounded-lg bg-primary text-primary-foreground"><Activity size={16} /></span><strong className="text-sm">Body Métrica FJ</strong></Link>
              <div className="max-w-sm"><div className="inline-flex items-center gap-2 rounded-full border border-white/18 bg-black/35 px-3 py-1.5 text-[11px] font-semibold backdrop-blur-md"><Sparkles size={13} /> Cadastro inteligente</div><h1 className="mt-4 font-display text-[2.35rem] font-semibold leading-[1.03] tracking-[-.04em]">Seu perfil começa com os dados certos.</h1><p className="mt-3 text-sm leading-6 text-white/78">Um cadastro único para personalizar metas, composição corporal, hidratação e estimativas da plataforma.</p><div className="mt-5 grid gap-2">{["Conta e identificação", "Dados para métricas", "Segurança e confirmação"].map((item, index) => <div key={item} className="flex items-center gap-3 rounded-xl border border-white/12 bg-black/28 px-3 py-2.5 backdrop-blur-sm"><span className="flex size-7 items-center justify-center rounded-lg bg-white/10 text-xs font-bold">{index + 1}</span><span className="text-xs font-medium text-white/88">{item}</span></div>)}</div></div>
          ...
          <main className="flex min-h-0 items-center justify-center bg-card px-4 py-5 sm:px-7 lg:px-8 lg:py-5 xl:px-10">
            <div className="w-full max-w-[680px]">
              <div className="mb-4 flex items-start justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[.16em] text-primary">Cadastro unificado</p><h2 className="mt-1 font-display text-2xl font-semibold tracking-[-.035em] sm:text-[1.85rem]">{step === 1 ? "Crie sua identificação" : "Complete seu perfil"}</h2><p className="mt-1 text-xs leading-5 text-muted-foreground">{step === 1 ? "CPF é validado e o e-mail precisa ser confirmado." : "Informações para métricas e segurança do acesso."}</p></div><div className="flex shrink-0 items-center gap-1.5 rounded-full border border-border bg-muted/45 px-2.5 py-1.5 text-[10px] font-bold text-muted-foreground"><span className={step >= 1 ? "text-primary" : ""}>01</span><span>/</span><span className={step >= 2 ? "text-primary" : ""}>02</span></div></div>
                  ...
                  {step === 1 ? (
                    <div className="rounded-2xl border border-border bg-background/55 p-3.5 sm:p-4">
                      <div className="grid gap-x-3 gap-y-3 sm:grid-cols-2">
                        ...
                        <TextField form={form} name="email" label="E-mail real" placeholder="voce@email.com" type="email" icon={<Mail size={15} />} maxLength={254} />
                        <FormField control={form.control} name="cpf" render={({ field }) => <FormItem><FormLabel className="text-xs">CPF</FormLabel><FormControl><Input className="h-10" inputMode="numeric" autoComplete="off" maxLength={14} placeholder="000.000.000-00" value={field.value} onChange={(e) => field.onChange(formatCpf(e.target.value))} /></FormControl><FormMessage className="text-[10px]" /></FormItem>} />
                      ...
                      <p className="mt-3 rounded-lg border border-primary/15 bg-primary/5 px-3 py-2 text-[10px] leading-4 text-muted-foreground">O CPF precisa passar pela validação dos dígitos verificadores. O e-mail será confirmado por um link enviado para a caixa de entrada; sem confirmação, a conta não é liberada.</p>
                    </div>
                  ) : (
                    <div className="rounded-2xl border border-border bg-background/55 p-3.5 sm:p-4"><div className="grid gap-x-3 gap-y-3 sm:grid-cols-2 lg:grid-cols-3"><SelectField form={form} name="goal" label="Objetivo principal" options={[...GOAL_OPTIONS]} /><SelectField form={form} name="activityLevel" label="Nível de atividade" options={[...ACTIVITY_OPTIONS]} /><div className="grid grid-cols-2 gap-3 sm:col-span-2 lg:col-span-1"><NumberField form={form} name="weight" label="Peso (kg)" min={25} max={400} step="0.1" /><NumberField form={form} name="height" label="Altura (cm)" min={100} max={250} step="1" /></div><div className="sm:col-span-1 lg:col-span-3"><div className="grid gap-3 sm:grid-cols-2"><PasswordField form={form} name="password" label="Senha" visible={showPassword} toggle={() => setShowPassword((v) => !v)} /><PasswordField form={form} name="confirmPassword" label="Confirmar senha" visible={showConfirmPassword} toggle={() => setShowConfirmPassword((v) => !v)} /></div></div></div><div className="mt-3 grid grid-cols-3 gap-1.5 rounded-xl border border-border bg-muted/30 p-2">{passwordChecks.map((item) => <div key={item.label} className="flex items-center gap-1.5 text-[10px] text-muted-foreground"><span className={`flex size-4 items-center justify-center rounded-full border ${item.ok ? "border-success/35 bg-success/12 text-success" : "border-border"}`}><Check size={9} /></span><span>{item.label}</span></div>)}</div></div>
                  )}

                  <div className="mt-4 flex gap-2.5">{step === 2 && <Button type="button" variant="outline" onClick={() => setStep(1)} className="h-11 rounded-xl px-4"><ArrowLeft size={15} className="mr-2" />Voltar</Button>}{step === 1 ? <Button type="button" onClick={goNext} className="h-11 flex-1 rounded-xl font-semibold">Validar e continuar<ArrowRight size={15} className="ml-2" /></Button> : <Button type="submit" disabled={isLoading} className="h-11 flex-1 rounded-xl font-semibold">{isLoading ? "Criando sua conta..." : "Criar conta"}{!isLoading && <ArrowRight size={15} className="ml-2" />}</Button>}</div>
...
function TextField({ form, name, label, placeholder, type = "text", icon, maxLength }: any) { return <FormField control={form.control} name={name} render={({ field }) => <FormItem><FormLabel className="text-xs">{label}</FormLabel><FormControl><div className="relative"><span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground">{icon}</span><Input type={type} inputMode={type === "email" ? "email" : undefined} autoComplete={type === "email" ? "email" : undefined} maxLength={maxLength} placeholder={placeholder} className="h-10 pl-9" {...field} onChange={(e) => field.onChange(type === "email" ? e.target.value.replace(/\s/g, "") : e.target.value)} onBlur={(e) => { field.onBlur(); if (type === "email") field.onChange(normalizeEmail(e.target.value)); }} /></div></FormControl><FormMessage className="text-[10px]" /></FormItem>} />; }
