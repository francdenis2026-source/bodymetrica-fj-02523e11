...
alter table public.profiles
  ...
  check (biological_sex is null or biological_sex in ('female','male','not_informed'));

alter table public.profiles
  drop constraint if exists profiles_registration_source_check;

alter table public.profiles
  add constraint profiles_registration_source_check
  check (registration_source in ('self','admin'));

create index if not exists profiles_cpf_digits_idx
...
begin
  ...
  if exists (
    ...
    where p.id <> new.id
      and regexp_replace(coalesce(p.cpf, ''), '\D', '', 'g') = normalized
  ) then
...
for each row execute function public.enforce_unique_normalized_cpf();
...
declare
  ...
  resolved text;
begin
  if position('@' in coalesce(_identifier, '')) > 0 then
    return lower(btrim(_identifier));
  end if;
